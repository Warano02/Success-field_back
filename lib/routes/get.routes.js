...
router.get('/randomquote', (req, res) => {
    ...
    if (!Key) {
        ...
        axios.get(`https://api.api-ninjas.com/v1/quotes?category=${category}`, {
            ...
            .catch(error => {
                res.status(500).json({ error: true, msg: "Une erreur est survenue de notre coté. Veuillez réesayer ulterieurement" })
...
router.get('/warano-qr-code',(req,res)=>{
    ...
    if (!Key) {
        ...
        .catch(err=>{
          ...
          res.status(500).json({error:true,msg:"Une erreur est survenue de notre coté. Veuillez réesayer plus tard."})
        })
    }
})
