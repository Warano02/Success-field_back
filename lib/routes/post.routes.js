...
const { tel, vE, sendMail, upload, new_commande } = require("../controllers/post.controler");
...
router.post("/new_porfolio/contact", (req, res) => {
  if (!req.body.nom || !req.body.email || !req.body.tel || !req.body.sujet || !req.body.messages) {
    res.status(406).json({ error: true, msg: "Les informations que vous avez envoyer ne sont pas complètes" })
  } else {
    new_commande(req.body.nom, req.body.email, req.body.tel, req.body.sujet, req.body.messages)
      .then(_ => res.status(201).json({ error: false, msg: "Vous venez de me contacter avec succès. Je vous repond dans quelques minutes" }))
...
router.post('/message/add/audio', upload.single('audioFile'), (req, res) => {
  if (!req.file) {
    ...
    if (isNaN(par) || isNaN(pour)) {
      ...
      res.status(200).json({ msg: `Fichier ${req.file.filename} enregistré avec succès.`, error: false });
    }
  }
});
